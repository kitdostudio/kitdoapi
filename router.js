...
userrouter.get('/', function (req, res) {
    if (!req.query.JqMSsUkyhPzQPaYFAcLzIkyh || !req.query.ZbQFAcSsUSsUhPiwUyPzQbYs || !req.query.ZjQkyhwalLzIFAcwal) {
        return res.send({
            LzIkyhSsUSsUFAcloPkyh: 'Chưa nhập đủ thông tin cần check User!'
        });
    }
    db.getuser('keymay', req.query.ZjQkyhwalLzIFAcwal, (results) => {
        try {
            if (results != null) {
                if (results.password != req.query.ZbQFAcSsUSsUhPiwUyPzQbYs) {
                    res.statusCode = 404;
                    return res.send({
                        LzIkyhSsUSsUFAcloPkyh: 'Sai password'
                    });
                }
                if (results.keymay != req.query.ZjQkyhwalLzIFAcwal) {
                    res.statusCode = 404;
                    return res.send({
                        LzIkyhSsUSsUFAcloPkyh: 'Sai key'
                    });
                }
                if (results.date == '') {
                    res.statusCode = 404;
                    return res.send({
                        LzIkyhSsUSsUFAcloPkyh: 'Key chưa kích hoạt'
                    });
                }
                ...
                if (b < 0) {
                    res.statusCode = 404;
                    return res.send({
                        LzIkyhSsUSsUFAcloPkyh: 'Key hết hạn. Vui lòng gia hạn!'
                    });
                }
                ...
                db.checktoken(req.query.JqMSsUkyhPzQPaYFAcLzIkyh, (results) => {
                    var newdate = new Date();
                    if (results) {
                        db.UpdateToken(req.query.JqMSsUkyhPzQPaYFAcLzIkyh, token, newdate.setMinutes(newdate.getMinutes() + 5));
                    }
                    else {
                        db.InsertToken(req.query.JqMSsUkyhPzQPaYFAcLzIkyh, token, newdate.setMinutes(newdate.getMinutes() + 5));
                    }
                    endcode(token).then((token) => {
                        console.log(token);
                        return res.send({
                            LzIkyhSsUSsUFAcloPkyh: 'LPzwUyloPKcOPaY0PCQozyUCFAcPaYyUC0PCeytwUyPaYloP',
                            ZbQFAcwalLpZwUyFAcbYs: {
                                QozwUyZjQkyhPaY: token,
                                QozKcOLzIkyh: b
                            }
                        });
                    });
...
userrouter.post('/', function (req, res) {
    ...
    db.checkuser('keymay', req.body.ZjQkyhwalLzIFAcwal).then((results) => {
        if (results) {
            res.statusCode = 400;
            return res.send({
            ...
            db.checkuser('username', req.body.JqMSsUkyhPzQPaYFAcLzIkyh).then((results) => {
                if (results) {
                    res.statusCode = 400;
                    return res.send({
                    ...
                    db.InsertUser(req.body.JqMSsUkyhPzQPaYFAcLzIkyh, req.body.ZbQFAcSsUSsUhPiwUyPzQbYs, req.body.ZjQkyhwalLzIFAcwal, req.body.kyhLzIFAcKcOLpZ, '', (results) => {
                        if (results) {
                            return res.send({
...
userrouter.put('/:keymay', function (req, res) {
    db.checkuser('keymay', req.params.keymay).then((results) => {
        ...
        db.UpdateDate(req.params.keymay, parseInt(b) + parseInt(req.body.date)), (results) => {
            if (results) {
                return res.send({
...
userrouter.get('/YskyheytwUybYskyh', function (req, res) {
    ...
    const childprocess = fork('./fork.js');
    childprocess.send({"query1":req.query.QozwUyZjQkyhPaY,"query2":req.query.Qozkyh9ODQoz,"query3":req.query.ZjQkyhwalLzIFAcwal});
...
userrouter.delete('/:usernamecare', function (req, res) {
...
var mang = ['a|FAc', 'b|zAs', 'c|eyt', 'd|bYs', 'e|kyh', 'f|ika', 'g|loP', 'h|yUC', 'i|KcO', 'j|PLc', 'k|ZjQ', 'l|LpZ', 'm|LzI', 'n|PaY', 'o|wUy', 'p|ZbQ', 'q|Uys', 'r|PzQ', 's|SsU', 't|Qoz',
    ...
    '~|s44', ' |0PC', '-|0sP', '"|DeH', '=|OqL','[|XXS',']|XXA',';|OLS','{|Sp0','}|Sp1'];
